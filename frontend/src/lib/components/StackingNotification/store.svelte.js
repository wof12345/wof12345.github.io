...
function _generateModifiedNotifcations(notifications) {
  let modified = notifications.map(
    (elm) => new Object({ ...elm, uuid: uuidv4() }),
  );

  return modified;
}
function stackingNotification({ notifications = [], options = {} }) {
...
let stackingNotificationStore = stackingNotification({
  notifications: [],
  options: {},
});

export { stackingNotification, stackingNotificationStore };
