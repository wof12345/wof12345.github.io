...
>
	...
	<div class="flex h-full items-center justify-center text-white">
		<div class="flex h-full w-full gap-4 md:gap-8 p-4 md:p-8 pl-12 md:pl-24">
			
			<!-- Mobile: Only show Details if project is selected -->
			<!-- Desktop: Always show Details (with a default if none selected) -->
			
			<div class="h-full flex-col overflow-hidden rounded-2xl bg-white p-4 md:p-6 shadow-xl dark:bg-gray-900 
				{selectedProject ? 'flex w-full md:w-[65%]' : 'hidden md:flex md:w-[65%]'}">
				
				{#if selectedProject}
					<button 
						class="md:hidden mb-4 flex items-center gap-2 text-blue-600 font-semibold"
						onclick={goBack}
					>
						← Back to Projects
					</button>
			...
			<div class="h-full flex-col gap-4 overflow-hidden rounded-2xl bg-white p-4 shadow-xl dark:bg-gray-900
